---
funding-events: []
last-updated: '2026-06-24'
launched: null
locations: []
milestones:
- source: '[[sources/cap/cap-2020]]'
  status: unverified
  target: Finish Smart City Strategy; include needs for monitoring.
  year: 2020
- source: '[[sources/cap/cap-2020]]'
  status: unverified
  target: Pilot sensors throughout the City with University of Michigan and other
    partners.
  year: 2021
- source: '[[sources/cap/cap-2020]]'
  status: unverified
  target: Install sensors and monitors throughout the City as an interconnected network.
  year: 2024
parent-strategy: '[[strategies/strategy-6-resilience]]'
partners:
- '[[actors/office-of-sustainability-and-innovations]]'
- '[[actors/office-of-emergency-management]]'
- '[[actors/university-of-michigan]]'
- '[[actors/huron-river-watershed-council]]'
party-responsible: '[[actors/ann-arbor-information-technology-unit]]'
related-strategies:
- '[[strategies/strategy-6-resilience]]'
source-first-seen: '[[sources/cap/cap-2020]]'
status: planned
tags:
- sensors
- environmental-monitoring
- smart-city
- air-quality
- flood-management
title: Implement Sensors to Monitor and Manage Heat, Air Quality, Waterways, and Flooding
type: initiative
---

This initiative focuses on deploying drones and sensors to collect real-time data on heat, air quality, waterways, and flooding in order to proactively understand risks and mitigate vulnerabilities across Ann Arbor ([[sources/cap/cap-2020|cap-2020]]). The [[actors/ann-arbor-information-technology-unit|Ann Arbor Information Technology Unit]] leads implementation, with collaborators including the [[actors/university-of-michigan|University of Michigan]], [[actors/huron-river-watershed-council|Huron River Watershed Council]], and stormwater, wastewater, and water system teams ([[sources/cap/cap-2020|cap-2020]]). Key assumptions include that any adopted technology can integrate with the City's existing IT infrastructure and that the various technologies can inter-operate ([[sources/cap/cap-2020|cap-2020]]). The ten-year estimated cost is $300,000, primarily benefiting local government staff and regional decision-makers ([[sources/cap/cap-2020|cap-2020]]).

The [[initiatives/environmental-sensors-monitoring|Environmental Sensors Monitoring]] initiative supports A2Zero resilience work by tracking local environmental conditions, including air quality, in Ann Arbor ([[sources/annual-reports/a2zero-year4|a2zero-year4]]). Ann Arbor's air quality monitoring network expanded in Year 3 with the installation of two additional monitors, bringing the city's total to four deployed sensors ([[sources/annual-reports/a2zero-year3|a2zero-year3]]). A public-facing data visualization platform was launched at www.a2gov.org/airdata to make monitoring data accessible to residents ([[sources/annual-reports/a2zero-year3|a2zero-year3]]). By Year Four, the city reported continuing air-quality monitoring, addressing reliability issues with monitoring equipment, and launching an air quality visualization map ([[sources/annual-reports/a2zero-year4|a2zero-year4]]). Installing at least four more air quality monitors throughout Ann Arbor was identified as a Year 4 priority in the A2ZERO Year 3 Annual Report ([[sources/annual-reports/a2zero-year3|a2zero-year3]]), and the Year Four report states that six additional air quality monitors were ordered for deployment throughout the city, likely in fall 2024 ([[sources/annual-reports/a2zero-year4|a2zero-year4]]). The Year 5 priorities call for achieving the air quality monitoring goals outlined in A2ZERO ([[sources/annual-reports/a2zero-year4|a2zero-year4]]).
