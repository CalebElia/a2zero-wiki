---
funding-events: []
last-updated: '2026-06-26'
launched: null
locations:
- '[[locations/ann-arbor]]'
milestones:
- source: '[[sources/annual-reports/a2zero-year2]]'
  status: unverified
  target: Complete design of pilot commercial Solarize program to support businesses
    with accessing solar
  year: 2022
parent-strategy: '[[strategies/strategy-1-renewable-grid]]'
partners:
- '[[actors/ann-arbor-washtenaw-2030-district]]'
party-responsible: '[[actors/city-of-ann-arbor]]'
related-strategies:
- '[[strategies/strategy-1-renewable-grid]]'
source-first-seen: '[[sources/annual-reports/a2zero-year2]]'
status: planned
tags:
- commercial-solar
- solarize
- business-outreach
- 2030-district
- solar-pv
title: Commercial Solarize Pilot Program
type: initiative
---

A pilot commercial Solarize program was initiated in Year 2 to extend the Solarize group-purchasing model to Ann Arbor businesses, developed in collaboration with the [[actors/ann-arbor-washtenaw-2030-district|2030 District]] ([[sources/annual-reports/a2zero-year2|a2zero-year2]]). The program was in the design phase as of the Year 2 report, representing an expansion of the residential-focused [[initiatives/solarize-ann-arbor|Ann Arbor Solarize Program]] into the commercial sector ([[sources/annual-reports/a2zero-year2|a2zero-year2]]).
