...
**Registry-alias noise, tracked separately.** One curated alias in `entity_aliases.json` remains over-broad — bare "the City" — and would inflate mention-count ordering in `scan-flagged`/`retrieve-for-context` without adding real recall value (a generic term matching everywhere doesn't help distinguish one entity from another), so it stays in `_SCAN_STOPLIST` (`pipeline/recall_scan.py`). "A2Zero"/"A2Zero Program"/"A2Zero Office" were removed from the stoplist once the `_ambiguous_terms` mechanism below existed to handle them properly instead of just dropping them.

### `_ambiguous_terms` — multi-candidate names, not silently dropped or guessed

Some real-world names are genuinely ambiguous between two legitimate, different-typed entities depending on sentence intent — not naming inconsistency to normalize away, but a real semantic split. Four confirmed families live in `registry/entity_aliases.json`'s reserved `_ambiguous_terms` list: "Ann Arbor" (the place vs. the municipal government acting as an actor), "A2Zero" (the plan itself vs. shorthand for its administering office, OSI), "Washtenaw County," and "Michigan"/"State of Michigan." Each entry has `aliases`, `candidates` (each `{"type", "canonical"}`), and a `default`.

**Two independent consumers read the same list, so the LLM's judgment and the mechanical scanner never contradict each other:**

- **Resolution (`pipeline/_aliases.py`)** — `resolve_slug`/`resolve_slug_for_title`/`fuzzy_resolve_slug_for_title` check `_ambiguous_terms` first. Given the LLM's own `proposed_type` (already threaded through both Pass 1.5 call sites), a type match returns that candidate; a type given but matching no candidate returns `None` (never force a redirect the LLM's own judgment contradicts — the normal fallback chain decides instead); no type given returns the entry's `default`. The Writer and chunk-extraction prompts both receive the same list rendered as prose (`build_ambiguous_terms_block`), the same bracketed-context convention as the relationship lexicon, so the LLM's type judgment is actually informed by the same candidates the resolver will check.
- **Recall scan (`pipeline/recall_scan.py`)** — has no LLM and cannot judge intent, so `build_ambiguous_scan_index` + `scan_source_for_known_entities`'s `ambiguous_index` parameter flag **every** candidate for a matched ambiguous name, tagged `"ambiguous": True` / `"ambiguous-with": [siblings]`, rather than guessing one or staying silently blind (the old stoplist behavior for the A2Zero variants). A name present in `ambiguous_index` is excluded from the plain single-slug matching pass even if it also happens to be an entity's own title (e.g. `locations/ann-arbor.md`'s title is literally "Ann Arbor") — once a term is known-ambiguous, it always routes through the multi-candidate path, never a silently-confident single match. `staleness_lint` findings for these hits carry a `[ambiguous — verify against source: also matches {siblings}]` annotation so human triage knows it's an unresolved multi-candidate hit, not a confident miss — consistent with the "loud, not silent" principle `context-dropped` already established.
