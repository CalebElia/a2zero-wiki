...
**What changed:**
- **`pipeline/recall_scan.py`** — new module implementing a deterministic recall backstop underneath the Comprehend/Plan architecture: `build_entity_name_index()` builds a `{name: slug}` index from every entity page's title, computed verb-prefix variants (CAP-2020's "Support X" → later reports' "X"), and `registry/entity_aliases.json` aliases; `scan_source_for_known_entities()` string-scans a source in a single pass (longest-name-first alternation, not one `findall` per name) and returns match counts per slug; `augment_integration_plan()` folds hits missing from the Comprehend plan into a new `scan-flagged` list plus appends them to `retrieve-for-context`, ordered by mention count, without ever touching `extends` — mechanical provenance stays visibly distinct from LLM judgment in the audit trail.
- **Regex/performance fix** (same day, same module): boundary matching switched from `\b` to lookarounds (`(?<!\w)...(?!\w)`) — `\b` after a closing paren never matches (39 of 653 real indexed names, mostly parenthetical acronyms, were silently unmatchable), and the scan moved from one `re.findall` per indexed name (~2s against the largest real source) to a single compiled alternation pass (0.31s against the same 278KB file).
- **`pipeline/orchestrator.py`** — the scan now runs immediately after `validate_plan_slugs` and before the plan is persisted, so `scan-flagged` and `context-dropped` both land in `integration-plans/<uuid>.json` as first-class, auditable fields. `RETRIEVE_TOKEN_BUDGET` (`pipeline/pass1a_comprehend.py`) raised 30,000 → 60,000 tokens to absorb scan-added bodies alongside LLM-flagged ones; any slug that still doesn't fit is recorded loudly in the plan's `context-dropped` field and printed as a warning at ingest time — never silently dropped.
- **Per-chunk scoped body injection** (`pipeline/pass2a_chunk_loop.py`) — the LDP chunk loop now re-scans each chunk's own text and injects only the retrieved bodies that chunk actually mentions, instead of injecting every retrieved body into every chunk prompt. The `[INTEGRATION PLAN]` block (including the uncapped `scan-flagged` awareness entries) stays doc-wide by design — awareness of an entity's correct slug must survive even when a chunk's local text doesn't re-trigger the scanner. Prompt assembly is stable-prefix-first (`system → plan → known entities` before per-chunk body/section content) so Anthropic/Azure prefix caching amortizes the doc-wide block across all chunk calls in an ingest.
- **Writer link-preservation rule** (`pipeline/pass1b_synthesize.py`, closes Task 121) — the Writer's system prompt now requires preserving every `[[...]]` wikilink already present in the existing Progress Synthesis text (rather than dropping links on paraphrase) and linking first mentions of any plan-listed entity (`extends`, `retrieve-for-context`, `scan-flagged`) using the plan's slug — previously the `stub_pages` linking rule only covered entities the Writer itself newly created.
- **`--staleness` lint mode** (`pipeline/phase_b_lint.py`) — `staleness_lint()` re-runs the same scanner against an already-ingested source and files a `STALE_ENTITY` finding for every matched entity whose page gained no citation to that source; defaults to the last `source-uuid` in `meta/ingest-stats.jsonl` when none is given. Findings carry a `[context-dropped at ingest]` annotation when the slug was recorded as budget-dropped at ingest time, so triage sees the knowingly-deprioritized tail first. `write_staleness_findings()` writes/replaces a dedicated section in `review-queue.md`. Informational only, no auto-fix — a mention can legitimately go uncited when a source repeats an already-recorded fact.
- **Verified against real data** (no re-ingest, no LLM calls): a 653-name index against the current wiki; a retroactive scan of the already-ingested Year 5 annual report matched 91 entities, 72 of which would have been `scan-flagged` beyond the recorded Comprehend plan — including `initiatives/bryant-neighborhood-decarbonization`, confirming this closes the exact regression that motivated the work (digest named only 76/497 entities; the Year 5 Comprehend plan covered 36 slugs; Bryant, mentioned 8× in the source, was in neither).
- **321 passed, 1 skipped.**

**Why:** The Comprehend/Plan architecture fixed cross-source fragmentation but left recall as a three-deep chain of LLM judgment (digest top-N compression → Comprehend plan selection → Writer's own entity-mention reasoning) with no deterministic backstop. Failures concentrated on mid-tier entities — mentioned often enough to matter, not prominent enough to survive a lossy compression or catch an LLM's attention against a long document — and were self-reinforcing (a dropped entity gets even less synthesis weight in the next digest rebuild). Comprehension stays LLM work; recall is now mechanical, with a loud, human-triaged instrument (staleness lint) for anything that still slips through.

**Spec:** `docs/architecture/deterministic-recall-floor.md`. **Plan:** `docs/superpowers/plans/2026-07-06-deterministic-recall-floor.md`. **Branch:** `feat/deterministic-recall`.

---

## 2026-07-02 — Moved wiki/meta/ to root-level meta/

**What changed:**
- `wiki/meta/` (6 pipeline-internal governance/audit files: `query-log.md`, `topic-candidates.md`, `schema-drift.md`, `synthesis-ghosts.log`, `ingest-stats.jsonl`, `relationship-lexicon.md`) moved to a root-level `meta/` directory, sibling to `wiki/`, `pipeline/`, `registry/`, `blackboard/` — same as `review-queue.md`'s existing location, via the same `Path(wiki_root).parent / "..."` pattern already used for it in `pipeline/phase_b_lint.py`.
- **Why:** these are pipeline governance/audit artifacts, not synthesized A2Zero knowledge content, but living inside the vault meant an agentic query session (which reads `wiki/index.md` first) could wander into them as if they were content. `wiki/index.md` had 3 manually-added links into `meta/*` files that directly exposed this — confirmed those links were never auto-generated by `rebuild_index()` and would've silently vanished on the next real rebuild anyway. `SCHEMA.md` already documented the intended design as "not ingest output, not in index"; the move makes that structurally true instead of just aspirational.
- Updated 4 pipeline files' path construction (`orchestrator.py`, `pass1b_synthesize.py`, `pass2b_extract.py`, `phase_c_synthesize.py`), removed the now-dead `"meta"` entry from `phase_b_lint.py`'s `ORPHAN_EXEMPT_DIRS`, updated `tests/test_run_ingest.py`'s fixture, and updated `CLAUDE.md`/`SCHEMA.md`'s directory-tree diagrams plus prose path references across `docs/architecture/*` and `docs/how-to-run-ingest.md`.
- Full test suite green throughout (279 passed, 1 skipped).

**Branch:** `refactor/meta-root-relocation`.
